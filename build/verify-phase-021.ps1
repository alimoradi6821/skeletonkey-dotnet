...
try {
    ...
    $dependencyRoot = Join-Path $repo "artifacts/workflows/phase-021-smoke"
    ...
    & dotnet $runnerDll run --file $invokeWorkflow --workflow-directory $dependencyRoot --execution-id phase-021-invocation-smoke
    if ($LASTEXITCODE -ne 0) { throw "Cross-workflow invocation smoke failed." }

    $checkpointRoot = Join-Path $repo "artifacts/checkpoints/phase-021-smoke"
    ...
    & dotnet $runnerDll run --file $workflow --execution-id phase-021-smoke --checkpoint-directory $checkpointRoot
    if ($LASTEXITCODE -ne 0) { throw "Durable checkpoint run smoke failed." }
    & dotnet $runnerDll resume --file $workflow --execution-id phase-021-smoke --checkpoint-directory $checkpointRoot
