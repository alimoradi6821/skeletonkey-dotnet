...
try {
    ...
    $pluginRoot = Join-Path $repo "artifacts/plugins/phase-022-smoke"
    if (Test-Path $pluginRoot) { Remove-Item $pluginRoot -Recurse -Force }
    New-Item $pluginRoot -ItemType Directory | Out-Null
    $pluginSource = Join-Path $repo "tests/SkeletonKey.Runner.Core.Tests/bin/Release/net10.0/SkeletonKey.Runner.Core.Tests.dll"
    ...
    & dotnet $runnerDll run --file $pluginWorkflow --plugin-directory $pluginRoot --execution-id phase-022-plugin-smoke
    if ($LASTEXITCODE -ne 0) { throw "Explicit plugin execution smoke failed." }

    $dependencyRoot = Join-Path $repo "artifacts/workflows/phase-022-smoke"
    ...
    & dotnet $runnerDll run --file $invokeWorkflow --workflow-directory $dependencyRoot --execution-id phase-022-invocation-smoke
    if ($LASTEXITCODE -ne 0) { throw "Cross-workflow invocation smoke failed." }

    $checkpointRoot = Join-Path $repo "artifacts/checkpoints/phase-022-smoke"
    ...
    & dotnet $runnerDll run --file $workflow --execution-id phase-022-smoke --checkpoint-directory $checkpointRoot
    if ($LASTEXITCODE -ne 0) { throw "Durable checkpoint run smoke failed." }
    & dotnet $runnerDll resume --file $workflow --execution-id phase-022-smoke --checkpoint-directory $checkpointRoot
