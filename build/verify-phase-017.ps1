...
try {
    ...
    $checkpointRoot = Join-Path $repo "artifacts/checkpoints/phase-017-smoke"
    ...
    & dotnet $runnerDll run --file $workflow --execution-id phase-017-smoke --checkpoint-directory $checkpointRoot
    if ($LASTEXITCODE -ne 0) { throw "Durable checkpoint run smoke failed." }
    & dotnet $runnerDll resume --file $workflow --execution-id phase-017-smoke --checkpoint-directory $checkpointRoot
