...
try {
    ...
    $checkpointRoot = Join-Path $repo "artifacts/checkpoints/phase-018-smoke"
    ...
    & dotnet $runnerDll run --file $workflow --execution-id phase-018-smoke --checkpoint-directory $checkpointRoot
    if ($LASTEXITCODE -ne 0) { throw "Durable checkpoint run smoke failed." }
    & dotnet $runnerDll resume --file $workflow --execution-id phase-018-smoke --checkpoint-directory $checkpointRoot
