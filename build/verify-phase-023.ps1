...
try {
    if (-not $IsWindows -and $PSVersionTable.PSEdition -eq "Core") {
        throw "Phase 23 verification must run on Windows."
    }
    ...
    if (-not $SkipAdvancedSmoke) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "Advanced Chromium and network interception smoke failed." }
    }
    ...
    $pluginRoot = Join-Path $repo "artifacts/plugins/phase-023-smoke"
    ...
    & dotnet $runnerDll run --file $pluginWorkflow --plugin-directory $pluginRoot --execution-id phase-023-plugin-smoke
    if ($LASTEXITCODE -ne 0) { throw "Explicit plugin execution smoke failed." }

    $dependencyRoot = Join-Path $repo "artifacts/workflows/phase-023-smoke"
    ...
    & dotnet $runnerDll run --file $invokeWorkflow --workflow-directory $dependencyRoot --execution-id phase-023-invocation-smoke
    if ($LASTEXITCODE -ne 0) { throw "Cross-workflow invocation smoke failed." }

    $checkpointRoot = Join-Path $repo "artifacts/checkpoints/phase-023-smoke"
    ...
    & dotnet $runnerDll run --file $workflow --execution-id phase-023-checkpoint-smoke --checkpoint-directory $checkpointRoot
    if ($LASTEXITCODE -ne 0) { throw "Durable checkpoint run smoke failed." }
    & dotnet $runnerDll resume --file $workflow --execution-id phase-023-checkpoint-smoke --checkpoint-directory $checkpointRoot
    ...
    if (-not $SkipDesktopSmoke) {
        ...
        & dotnet $runnerDll run --file $desktopWorkflow --locator-directory $desktopDirectory --execution-id phase-023-desktop-smoke
