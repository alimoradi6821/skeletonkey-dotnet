...
Lifetimes are `execution`, `host`, and `invocation`. Execution-scoped resources may be shared across a root execution when explicitly mapped. Host-scoped resources are owned by an explicit host registry and may be reused across independent executions. Invocation-scoped resources belong to one workflow invocation and are not inherited by child workflows.
...
Node resource slots differ from acquired resource leases. A node definition may declare a slot, the default analyzer matches that slot to a workflow `$resource` wrapper, and the default planner records a planned resource use. A handler invocation acquires an `INodeResourceLease` only through slot-scoped access. The host supplies explicit providers and the runtime creates instances lazily. Invocation/execution resources remain execution-owned; host-lifetime resources are registry-owned and are not disposed by an ordinary execution. Cross-execution access to the same host resource uses shared instance-level lease coordination.
...
Host-lifetime resources are intentionally excluded from execution checkpoints. They are resolved again from the current host registry after resume and are disposed only when recycled or when the host registry itself shuts down. See [Host-Lifetime Resources and Health 0.1](host-lifetime-resources-0.1.md).
